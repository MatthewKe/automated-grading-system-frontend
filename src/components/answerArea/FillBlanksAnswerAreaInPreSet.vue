...
function startDrag(event) {
  event.dataTransfer.setData("text/plain", "new a fillBlanksAnswerArea")
}
...
onMounted(() => {
  const canvas = document
      .getElementById('fillBlanksAnswerAreaInPreSet')
  ...
  ctx.fillText('填空题', canvas.width / 2, canvas.height / 2)
})
...
<template>
  <div
      id="fillBlanksAnswerAreaInPreSet"
      draggable="true"
      @dragstart="startDrag"
      @dragend="endDrag"
  >
